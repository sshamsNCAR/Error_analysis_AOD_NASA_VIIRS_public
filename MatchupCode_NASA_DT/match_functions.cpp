...
int getAerAod(string fileName, int matchDay, int dayShift, float timeWindow[2], vector<AerAod>& obs)
{
   ...
   if (staFile.is_open()) {
      ...
      while(getline(staFile, oneLine)) {   /* loop over measurements for one station */
         // Skip lines without commas (invalid data)
         end = oneLine.find(',', 0);
         ...
         // Extract the date field (day:month:year)
         // string dateField = oneLine.substr(0, end);
         // int day, month, year;
         // sscanf(dateField.c_str(), "%d:%d:%d", &day, &month, &year);

         // // Debug: Print the extracted date components
         // cout << "Extracted date - Day: " << day << ", Month: " << month << ", Year: " << year << endl;

         // // Adjust the year if necessary (e.g., if year is two digits, add 2000)
         // if (year < 100) year += 2000;

         // // Debug: Print the adjusted year
         // cout << "Adjusted Year: " << year << endl;

         // Move to the time field (hh:mm:ss)
         start = end + 1;
         end = oneLine.find(',', start);
         // Extract the time field (hh:mm:ss)
         // string timeField = oneLine.substr(start, end - start);
         // cout << "Extracted time field: " << timeField << endl;

         // Move to the fractional day of the year field
         time = (float)atof((oneLine.substr(start, end-start)).c_str());
         // Debug: Print the extracted fractional time
         // cout << "Extracted fractional time: " << time << endl;

         // if ((int)(time) != matchDay) continue;
         // Check if the day matches the matchDay
         if ((int)(time) != matchDay) {
            // cout << "Skipping line (time does not match matchDay): " << time << endl;
         ...
         if (time >= timeWindow[0] && time <= timeWindow[1]) {
            ...
            //  Debug: Print the extracted AOD values
            // cout << "Extracted AOD values for time " << oneObs.time << ": ";
            // for (int i = 0; i < NUM_AERONET_WAVELENGTH; i++) {
            //    cout << oneObs.aods[i] << " ";
            // }
            // cout << endl;
     ...
     // Debug print: Indicate which file failed to open
     cout << "getAerAod(): Cannot open AERONET file: " << fileName << endl;
     return PROC_FAIL;
   }
} 
                     ...
                     float& westStaLon, float& eastStaLon, float& northStaLat, float& southStaLat)
{
   ...
   // cout << "Station: " << sta.staName << ", Valid Measurements: " << ntm 
...
int getAerInfo(map<string,float*>& aerInfo)
{
   ...
   if (aerInfoFile.is_open()) {
      ...
      while (getline(aerInfoFile, oneLine)) {
         ...
         // cout << "Processing AERONET station txt: " << oneLine << endl;
         int start, end;
         string staName;
         float  *lonLat;
         // while(getline(aerInfoFile, oneLine)) {
         ...
         for (int i=0; i<3; i++) {
            ...
            // // Debug print statement for station info
            // cout << "Station: " << staName 
            // << ", Longitude: " << lonLat[0] 
            // << ", Latitude: " << lonLat[1] << endl;
         }
         // cout << "Processing AERONET station: " << staName << lonLat[0] <<lonLat[1] << endl;
      }
   }
   else {
...
{  
   ...
   command = "ls "+AOD_PATH+year+"/AERDT*.A"+yyyyddd+".*.nc";
...
{ 
   ...
   for (map<float,string>::iterator it=granInfo.begin(); it!=granInfo.end(); ++it) {
      // cout << "Processing granule: " << it->second << " at time: " << it->first << endl;
      ...
      // cout << "Granule Boundaries:" << endl;
      // cout << "West Longitude: " << westGranLon << ", East Longitude: " << eastGranLon << endl;
      // cout << "North Latitude: " << northGranLat << ", South Latitude: " << southGranLat << endl;
   
      // cout << "Station Matching Boundaries:" << endl;
      // cout << "West Longitude: " << westStaLon << ", East Longitude: " << eastStaLon << endl;
      // cout << "North Latitude: " << northStaLat << ", South Latitude: " << southStaLat << endl; 
      ...
      for (vector<OneDayAerAod>::iterator ia=aerData.begin(); ia!=aerData.end(); ++ia) {
         ...
         // cout << "Before pushing to matchData: nPixs = " << mr.nPixs << endl;
         /* insert to the matching vector */
