...
{    
   ...
   for (vector<MatchupRecord>::iterator im = matchData.begin(); im != matchData.end(); ++im) {
      ...
      std::ostringstream aod550_values, qf_values, lndSea_values;
      ...
      for (int i = 0; i < im->nPixs; ++i) {
         ...
         // Append lndSea values
         lndSea_values << im->lndSea[i];
         if (i < im->nPixs - 1) {
               lndSea_values << ";";  // Separate values with a semicolon
         }
      }
      output << yyyymmdd << ","  // Add the date
            ...
            << "\"" << lndSea_values.str() << "\"\n";  // Land/sea flag for all pixels (enclosed in quotes)
   }
   // Write match-up data if all the pixels data are needed
   // for (vector<MatchupRecord>::iterator im = matchData.begin(); im != matchData.end(); ++im) {
   //    for (int i = 0; i < im->nPixs; ++i) {
   //          output << yyyymmdd << ","  // Add the date
   //                 << im->nMeas << ","
